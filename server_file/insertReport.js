...
router.post('/insertReport', function(req, res, next) {
    ...
    let rp = req.body;
    let reportParams = [String(rp.CaseID), String(rp.StudyDate), String(rp.Name), Number(rp.Age), String(rp.Sex), String(rp.DOB), Number(rp.Height), 
        ...
        Number(rp.MAT), Number(rp.HA), Number(rp.HAT), Number(rp.LA), Number(rp.LH), Number(rp.MeanSpO2), Number(rp.MeanDesat), 
    ...
    .then(conn => {
        conn.query("insert into report(" +
            "CaseID, StudyDate, Name, Age, Sex, DOB, Height, Weight, BMI, Neck, AHI, AI, HI, OI, CI, MI, AHI_Supine, AHI_NSupine, " +
            ...
            "SleepLatency, Stage3, Efficiency, ArousalIndex, OA, OAT, CA, CAT, MA, MAT, HA, HAT, LA, LH, MeanSpO2, " +
